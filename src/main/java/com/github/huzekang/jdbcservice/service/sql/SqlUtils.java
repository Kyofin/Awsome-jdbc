 ...
 * 处理sql和jdbc的核心类。
 *
 *      依赖于jdbcDataSource存储创建的druid数据源。
 *      依赖于SourceUtils 处理druid数据源的释放还有ResultSet后的close等操作
 *
 * 参考使用方法：com.github.huzekang.jdbcservice.SqlUtilsTest
 */
@Slf4j
@Component
@Scope("prototype")
public class SqlUtils {
    ...
    public PaginateWithQueryColumns query4Paginate(String sql, int pageNo, int pageSize, int totalCount, int limit, Set<String> excludeColumns) throws Exception {
        ...
        String md5 = MD5Util.encode(sql + pageNo + pageSize + limit);
    ...
    public static String getCountSql(String sql) {
        try {
            ...
            return String.format(Consts.QUERY_COUNT_SQL, select.toString());
    ...
    private static void getFromItemName(Set<String> columnPrefixs, FromItem fromItem) {
        ...
        if (alias != null) {
            if (alias.isUseAs()) {
                columnPrefixs.add(alias.getName().trim() + Consts.DOT);
            } else {
                columnPrefixs.add(alias.toString().trim() + Consts.DOT);
            }
        } else {
        ...
        if (!CollectionUtils.isEmpty(columnPrefixs)) {
            for (String prefix : columnPrefixs) {
                if (columnLable.startsWith(prefix)) {
                    return columnLable.replaceFirst(prefix, Consts.EMPTY);
                } else if (columnLable.startsWith(prefix.toLowerCase())) {
                    return columnLable.replaceFirst(prefix.toLowerCase(), Consts.EMPTY);
                } else if (columnLable.startsWith(prefix.toUpperCase())) {
                    return columnLable.replaceFirst(prefix.toUpperCase(), Consts.EMPTY);
                }
            }
        }
        return columnLable;
                ...
                if (null != tables) {
                    ...
                    while (tables.next()) {
                        ...
                        if (!StringUtils.isEmpty(name)) {
                            ...
                            tableList.add(new QueryTable(name, type));
                        }
                    }
                }
                tables.close();
    ...
    public static void checkSensitiveSql(String sql) throws ServerException {
        Matcher matcher = Consts.PATTERN_SENSITIVE_SQL.matcher(sql.toLowerCase());
        if (matcher.find()) {
    ...
    public static String getKeywordPrefix(String jdbcUrl, String dbVersion) {
        ...
        return StringUtils.isEmpty(keywordPrefix) ? Consts.EMPTY : keywordPrefix;
    }
    ...
    public static String getKeywordSuffix(String jdbcUrl, String dbVersion) {
        ...
        return StringUtils.isEmpty(keywordSuffix) ? Consts.EMPTY : keywordSuffix;
    }
    ...
    public static String getAliasPrefix(String jdbcUrl, String dbVersion) {
        ...
        return StringUtils.isEmpty(aliasPrefix) ? Consts.EMPTY : aliasPrefix;
    }
    ...
    public static String getAliasSuffix(String jdbcUrl, String dbVersion) {
        ...
        return StringUtils.isEmpty(aliasSuffix) ? Consts.EMPTY : aliasSuffix;
    }
    ...
    public static String filterAnnotate(String sql) {
        sql = Consts.PATTERN_SQL_ANNOTATE.matcher(sql).replaceAll("$1");
        sql = sql.replaceAll(Consts.NEW_LINE_CHAR, Consts.SPACE).replaceAll("(;+\\s*)+", Consts.SEMICOLON);
        return sql;
    }
    ...
    private static FromItemVisitor getFromItemTableName(Set<String> set) {
        return new FromItemVisitor() {
            ...
            public void visit(Table tableName) {
                set.add(tableName.getName() + Consts.DOT);
            }
    ...
    public static final class SqlUtilsBuilder {
        ...
        public SqlUtils build() throws ServerException {
            log.info("使用:{}生成一个sqlUtils对象返回",this.toString());
